...
HOST = "http://x"
CONTEXT_HOST = "http://cached-host"
ENVIRONMENT_HOST_VALUE = "http://env-host"
...
def test_explicit_api_key_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(ENVIRONMENT_API_KEY, "should-not-be-used")
    monkeypatch.setenv(ENVIRONMENT_BEARER_TOKEN, "should-not-be-used")
    cred = resolve_credential(HOST, api_key="explicit-key")
...
def test_explicit_token_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(ENVIRONMENT_BEARER_TOKEN, "should-not-be-used")
    cred = resolve_credential(HOST, token="explicit-token")
...
def test_api_key_beats_token_in_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(ENVIRONMENT_API_KEY, "key")
    monkeypatch.setenv(ENVIRONMENT_BEARER_TOKEN, "tok")
    assert resolve_credential(HOST).scheme is SecurityScheme.apikey


def test_cached_login_used_when_env_empty(tmp_path: Path) -> None:
    _write_login(tmp_path, "prod", "cached-jwt", timedelta(hours=1))
    cred = resolve_credential(HOST, sources=[])
...
def test_expired_cached_login_is_skipped(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuthError, match="No Marmot credentials found"):
        resolve_credential(HOST, sources=[])
...
def test_cached_login_follows_selected_context(
    ...
    monkeypatch.setenv(ENVIRONMENT_MARMOT_CONTEXT, "missing")
    with pytest.raises(AuthError, match="No Marmot credentials found"):
        resolve_credential(HOST, sources=[])
...
def test_workload_source_runs_when_no_other_credential(httpx_mock: object) -> None:
    ...
    cred = resolve_credential(HOST, sources=[_StaticSource("subject-jwt")])
