...
tools are driven by a fake async reader and the handler by a fake async registry.
"""
...
@pytest.mark.asyncio
async def test_search_tool_flattens_results(reader: _FakeReader) -> None:
    out = await _tools_by_name(reader)["search_catalog"].ainvoke({"query": "orders", "limit": 5})
...
@pytest.mark.asyncio
async def test_lookup_tool_returns_none_when_absent(reader: _FakeReader) -> None:
    tool = _tools_by_name(reader)["lookup_asset"]
    found = await tool.ainvoke({"asset_type": "Table", "service": "postgres", "name": "orders"})
    assert found["mrn"] == ORDERS_MRN
    assert (
        await tool.ainvoke({"asset_type": "Table", "service": "postgres", "name": "nope"}) is None
    )


@pytest.mark.asyncio
async def test_handler_registers_agent_on_first_chain_start(registry: _FakeRegistry) -> None:
...
@pytest.mark.asyncio
async def test_handler_emits_declared_edges_for_tagged_tools(registry: _FakeRegistry) -> None:
...
@pytest.mark.asyncio
async def test_handler_records_run_with_tool_calls(registry: _FakeRegistry) -> None:
    ...
    await handler.on_tool_end("rows", run_id=tool_run, parent_run_id=root)
...
@pytest.mark.asyncio
async def test_handler_mines_lookup_tool_output_for_observed_assets(
    ...
    await handler.on_tool_end({"mrn": ORDERS_MRN}, run_id=tool_run, parent_run_id=root)
    await handler.on_chain_end({}, run_id=root)
...
@pytest.mark.asyncio
async def test_handler_records_error_status_on_chain_error(registry: _FakeRegistry) -> None:
    handler = MarmotCallbackHandler(registry, name="orders-analyst")
    root = uuid4()

    await handler.on_chain_start({}, {}, run_id=root)
    await handler.on_chain_error(ValueError("boom"), run_id=root)
...
@pytest.mark.asyncio
async def test_handler_counts_tokens_across_llm_calls(registry: _FakeRegistry) -> None:
    handler = MarmotCallbackHandler(registry, name="orders-analyst")
    root = uuid4()

    await handler.on_chain_start({}, {}, run_id=root)
    await handler.on_llm_end(
        _llm_result_with_usage_metadata(11, 5), run_id=uuid4(), parent_run_id=root
    )
    await handler.on_llm_end(
        _llm_result_with_usage_metadata(11, 5), run_id=uuid4(), parent_run_id=root
    )
...
@pytest.mark.asyncio
async def test_handler_falls_back_to_llm_output_token_usage(registry: _FakeRegistry) -> None:
    ...
    await handler.on_chain_start({}, {}, run_id=root)
    await handler.on_llm_end(result, run_id=uuid4(), parent_run_id=root)
    await handler.on_chain_end({}, run_id=root)
...
@pytest.mark.asyncio
async def test_record_source_attributes_a_runtime_mrn(registry: _FakeRegistry) -> None:
    handler = MarmotCallbackHandler(registry, name="orders-analyst")
    root = uuid4()
