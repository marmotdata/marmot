// Code generated by go-swagger; DO NOT EDIT.

package models

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag/jsonutils"
)

// AssetExternalLink asset external link
//
// swagger:model AssetExternalLink
type AssetExternalLink struct {

	// icon
	Icon string `json:"icon,omitempty"`

	// name
	Name string `json:"name,omitempty"`

	// url
	URL string `json:"url,omitempty"`
}

// Validate validates this asset external link
func (m *AssetExternalLink) Validate(_ strfmt.Registry) error {
	return nil
}

// ContextValidate validates this asset external link based on context it is used
func (m *AssetExternalLink) ContextValidate(_ context.Context, _ strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *AssetExternalLink) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return jsonutils.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AssetExternalLink) UnmarshalBinary(b []byte) error {
	var res AssetExternalLink
	if err := jsonutils.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
