...
// New creates a new mcp API client with basic auth credentials.
//
...
// New creates a new mcp API client with a bearer token for authentication.
//
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
...
func (a *Client) PostMcpContext(ctx context.Context, params *PostMcpParams, opts ...ClientOption) (*PostMcpOK, error) {
	...
	msg := fmt.Sprintf("unexpected success response for postMcp: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
...
type innerParams struct {
	...
	// Deprecated: use the operation call with context to pass the context instead of [McpParams].
	ctx context.Context
}
