...
func (a *Client) GetUIConfigContext(ctx context.Context, params *GetUIConfigParams, opts ...ClientOption) (*GetUIConfigOK, error) {
	...
	success, ok := result.(*GetUIConfigOK)
	if ok {
		return success, nil
	}
	...
	msg := fmt.Sprintf("unexpected success response for getUIConfig: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
