...
func Meta() pluginsdk.Meta {
	return pluginsdk.Meta{
		...
		AssetSchemas: []pluginsdk.AssetSchema{
			pluginsdk.AssetSchemaOf(OpenMetadataFields{}, "OpenMetadata",
				"OpenMetadataFields records where in OpenMetadata an asset came from. It is set on every asset this plugin creates."),
			pluginsdk.AssetSchemaOf(CurationFields{}, "Curation",
				"The curated context OpenMetadata holds that a Marmot ingestion run cannot yet create as first class objects, so it travels with the asset instead."),
			pluginsdk.AssetSchemaOf(TableFields{}, "Table",
				"Metadata on tables, views and stored procedures. The field names match Marmot's own database plugins so a merged asset reads the same whichever run contributed it."),
			pluginsdk.AssetSchemaOf(TopicFields{}, "Topic",
				"Metadata on messaging topics."),
			pluginsdk.AssetSchemaOf(ContainerFields{}, "Container",
				"Metadata on object storage containers."),
			pluginsdk.AssetSchemaOf(DashboardFields{}, "Dashboard",
				"Metadata on dashboards, charts and dashboard data models."),
			pluginsdk.AssetSchemaOf(PipelineFields{}, "Pipeline",
				"Metadata on pipelines and their tasks."),
			pluginsdk.AssetSchemaOf(MLModelFields{}, "MLModel",
				"Metadata on machine learning models."),
			pluginsdk.AssetSchemaOf(SearchIndexFields{}, "Search Index",
				"Metadata on search indices."),
			pluginsdk.AssetSchemaOf(APIFields{}, "API",
				"Metadata on API collections and endpoints."),
			pluginsdk.AssetSchemaOf(DriveFields{}, "Drive",
				"Metadata on drive directories, files, spreadsheets and worksheets."),
		},
	}
}
