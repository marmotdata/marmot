...
func TestValidateWithProviderFederates(t *testing.T) {
	s := &Source{}
	out, err := s.Validate(pluginsdk.RawConfig{"project_id": "p", "locations": []any{"europe-west4"}, "workload_identity_provider": "//iam.googleapis.com/" + testProvider})
	if err != nil {
...
func TestValidateRefusesProviderWithKey(t *testing.T) {
	_, err := (&Source{}).Validate(pluginsdk.RawConfig{"project_id": "p", "locations": []any{"europe-west4"}, "workload_identity_provider": testProvider, "credentials_json": "{}"})
...
func TestValidateWithKeyDoesNotFederate(t *testing.T) {
	out, err := (&Source{}).Validate(pluginsdk.RawConfig{"project_id": "p", "locations": []any{"europe-west4"}, "credentials_json": "{}"})
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
