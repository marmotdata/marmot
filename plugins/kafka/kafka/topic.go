...
func (s *Source) createTopicAsset(ctx context.Context, topic string) (pluginsdk.Asset, error) {
	metadata := make(map[string]interface{})
	metadata["topic_name"] = topic

	topicDetails, err := s.getTopicDetails(ctx, topic)
	if err != nil {
		return pluginsdk.Asset{}, fmt.Errorf("getting topic details: %w", err)
	}
...
// kafkaConfigToMetadataMapping normalises Kafka's dotted config keys into
