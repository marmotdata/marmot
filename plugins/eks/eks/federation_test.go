...
func federatedConfig(extra map[string]any) pluginsdk.RawConfig {
	...
	return pluginsdk.RawConfig{"eks_cluster_name": "prod", "credentials": creds}
}
...
func TestValidateWithoutRoleDoesNotFederate(t *testing.T) {
	out, err := (&Source{}).Validate(pluginsdk.RawConfig{"eks_cluster_name": "prod", "credentials": map[string]any{"region": "eu-west-1"}})
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
