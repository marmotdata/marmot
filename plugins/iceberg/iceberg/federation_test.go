...
func federatedConfig(extra map[string]any) pluginsdk.RawConfig {
	...
	return pluginsdk.RawConfig{"uri": "https://catalog.example", "credentials": creds}
}
...
func TestValidateWithoutRoleDoesNotFederate(t *testing.T) {
	out, err := (&Source{}).Validate(pluginsdk.RawConfig{"uri": "https://catalog.example", "credentials": map[string]any{"region": "eu-west-1"}})
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
