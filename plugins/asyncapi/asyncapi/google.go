...
func (s *Source) createGooglePubSubTopic(doc *asyncapi3.Document, channelName string, channel *asyncapi3.Channel, binding *googlepubsub.ChannelBinding) pluginsdk.Asset {
	name := channelName
	if binding.Topic != "" {
		name = binding.Topic
	}
