...
func TestValidateWithProviderFederates(t *testing.T) {
	out, err := (&Source{}).Validate(pluginsdk.RawConfig{"project_id": "p", "location": "europe-west4", "cluster": "prod", "credentials": map[string]any{"workload_identity_provider": testProvider}})
	if err != nil {
...
func TestValidateRefusesProviderWithKey(t *testing.T) {
	_, err := (&Source{}).Validate(pluginsdk.RawConfig{"project_id": "p", "location": "europe-west4", "cluster": "prod", "credentials": map[string]any{"workload_identity_provider": testProvider, "credentials_json": "{}"}})
...
func TestValidateWithKeyDoesNotFederate(t *testing.T) {
	out, err := (&Source{}).Validate(pluginsdk.RawConfig{"project_id": "p", "location": "europe-west4", "cluster": "prod", "credentials": map[string]any{"credentials_json": "{}"}})
	if err != nil {
		t.Fatalf("validate: %v", err)
	}
