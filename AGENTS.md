...
- Core membership and `required` are independent. Validity is checked on the final persisted state (including ingest and dedicated mutations); completeness (missing `required` values) is reported and never blocks a write. Preserve unknown metadata and MRNs.
