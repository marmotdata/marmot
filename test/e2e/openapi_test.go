...
var (
	...
	testPetStoreSpecPath = "testdata/openapi-petstore.yaml"
)
...
func TestOpenAPIIngestionWithTagsFromSpec(t *testing.T) {
	specsDir, err := filepath.Abs(containerSpecsPath)
	require.NoError(t, err, "Failed to get absolute path")
	...
	specBytes, err := os.ReadFile(testPetStoreSpecPath)
    	require.NoError(t, err, "Failed to read spec file from testdata")
...
`, containerSpecsPath)
	...
	openapiService := utils.FindAssetByName(resp.Payload.Assets, "OpenAPI Petstore")
	require.NotNil(t, openapiService, "OpenAPI service not found")
	assert.Equal(t, "Service", openapiService.Type)
	assert.Contains(t, openapiService.Tags, "openapi")
	assert.Contains(t, openapiService.Tags, "api")
	assert.Contains(t, openapiService.Tags, "pet")
	assert.Contains(t, openapiService.Tags, "store")
	assert.Contains(t, openapiService.Tags, "user")
	assert.Equal(t, len(openapiService.Tags), 5)
