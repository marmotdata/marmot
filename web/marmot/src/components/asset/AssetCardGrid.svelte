	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
		{#if isLoading}
			{#each Array(6) as _, i (i)}
				<div
					class="bg-earthy-brown-50 dark:bg-gray-800 shadow-md dark:shadow-gray-700 rounded-lg p-4 flex flex-col items-center animate-pulse"
				>
		...
		{:else}
			{#each Object.entries(items) as [key, value] (key)}
				<a
					href={resolve(`/discover?${filterType}=${key}`)}
