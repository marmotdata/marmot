<script lang="ts">
	...
	let { product }: { product: DataProduct } = $props();

	// GET /api/v1/metamodel?kind=data_product is cached by fetchMetamodel; every blade instance shares one request.
	...
	const neededOwnerIds = $derived.by(() => {
		...
		for (const field of fields) {
			...
			const value = readMetadataValue(product.metadata, field.storage);
