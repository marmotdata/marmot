	...
	<div class="flex flex-wrap gap-4">
		{#if isLoading}
			{#each Array(12) as _, i (i)}
				<div
					class="bg-earthy-brown-50 dark:bg-gray-900 shadow-md dark:shadow-gray-700 rounded-full px-4 py-2 animate-pulse"
				>
		...
		{:else}
			{#each sortedTags as [tag, count] (tag)}
				<a
					href={resolve(`/discover?tags=${encodeURIComponent(tag)}`)}
