		...
		{#each tags as tag}
			...
			>
				...
				{#if canEdit}
					...
					>
						<IconifyIcon icon="material-symbols:close-small-rounded" class="w-4 h-4" />
					</button>
				{/if}
			</span>
		{/each}
		...
		{#if showTagInput}
			<div class="inline-flex items-center gap-1 bg-gray-50 dark:bg-gray-700 rounded-md px-2 py-1">
				...
				<button
					...
					title="Add"
				>
					<IconifyIcon icon="material-symbols:check-rounded" class="w-4 h-4" />
				</button>
				<button
					...
					title="Cancel"
				>
					<IconifyIcon icon="material-symbols:close-rounded" class="w-4 h-4" />
				</button>
			</div>
		{/if}
