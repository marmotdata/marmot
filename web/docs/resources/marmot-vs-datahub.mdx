...
<Head>
  ...
  <meta name="twitter:image" content="https://marmotdata.io/img/resources/marmot-vs-datahub.png" />
  ...
  <meta property="og:image" content="https://marmotdata.io/img/resources/marmot-vs-datahub.png" />
...
Marmot ships 70+ plugins spanning databases, warehouses, lakes, object storage, streaming, orchestration, compute, BI and search. For anything without a plugin yet, Marmot's official Terraform and Pulumi providers (`marmot_asset`, `marmot_lineage`) populate assets and lineage straight from the infrastructure you already define, so a source still lands in the catalog from code you are writing anyway. DataHub leans on YAML ingestion recipes and its SDK instead. DataHub's pre-built library is still the larger of the two, but for a typical stack the question is now whether your sources are covered rather than how the totals compare, and at 70+ plugins plus catalog-as-code they usually are.
