...
<Head>
  ...
  <meta name="twitter:image" content="https://marmotdata.io/img/resources/marmot-vs-openmetadata.png" />
  ...
  <meta property="og:image" content="https://marmotdata.io/img/resources/marmot-vs-openmetadata.png" />
...
Marmot ships 70+ plugins spanning databases, warehouses, lakes, object storage, streaming, orchestration, compute, BI and search. For anything without a plugin yet, Marmot's official Terraform and Pulumi providers (`marmot_asset`, `marmot_lineage`) populate assets and lineage straight from the infrastructure you already define, so a source still lands in the catalog from code you are writing anyway. OpenMetadata still has the larger library, and if raw connector count is your deciding factor it wins the column. What has changed is that Marmot's 70+ plugins cover the same categories, so for most stacks the choice comes down to whether you want to run an ingestion framework rather than whether your sources are supported.
