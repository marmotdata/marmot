  ...
  <script type="application/ld+json">
    {JSON.stringify({
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 7, "name": "Amundsen", "url": "https://github.com/amundsen-io/amundsen" },
...
**Here is how the eight catalogs compare as AI context layers, as of September 2026.** MCP cells reflect what we could confirm from each vendor's current documentation and repositories. Where a vendor is actively shipping MCP, we say so; we have not assumed any tool "can't" just to flatter the column. One note on the table below: Amundsen was archived by its maintainers on 10 September 2026 and is included here for teams who still run it.
...
|------|-------------|-----|---------------------|------------------------|-------------------|------------------|------------|-------------|----------|
| **[Marmot](https://github.com/marmotdata/marmot)** | Native (built in) | Full | Yes, REST | Yes, via MCP, CLI, SDK and API | Postgres only (Elasticsearch optional) | Single Go binary | 70+ plugins + IaC | Yes (MIT) | Native MCP with the smallest footprint |
...
| **[Amundsen](https://github.com/amundsen-io/amundsen)** | None | None | Yes, REST | Yes (in UI), limited via API | Neo4j or Atlas, Elasticsearch | Multi-service | Databuilder, unmaintained | Yes (Apache 2.0), archived | Existing estates planning a migration |
...
- **Fewer connectors than the largest libraries.** 70+ plugins against 120+ for OpenMetadata, covering databases, warehouses, lakes, object storage, streaming, orchestration, compute, BI and search. For anything without a plugin yet, the Terraform and Pulumi providers or the Go, TypeScript and Python SDKs populate assets and lineage straight from your existing infrastructure and code, so the gap is bridged rather than left open.
...
**Best for:** teams who already run it and need a migration path. It is not a starting point in 2026.

Amundsen, originally from Lyft, helped define modern data discovery with strong search over a metadata graph, and it was donated to the LF AI & Data Foundation as an incubation project in 2020. Development slowed through 2024, and on 10 September 2026 the maintainers [archived the repository](https://github.com/amundsen-io/amundsen), which is now read-only. The amundsen.io domain no longer serves the project either, so the archived repository is the only reliable reference left.

**Amundsen pros:**

- **Strong search-led discovery** over a graph backed by Neo4j or Atlas and Elasticsearch, which is what made it widely adopted in the first place.
- **REST API and lineage** rendered in the UI.
- **Apache 2.0 licensed**, and the code stays available for historical purposes.

**Amundsen cons:**

- **Archived and unmaintained.** No new features, no bug fixes and no security patches. Anything you run from here, you are maintaining yourself.
- **No MCP, first-class or otherwise.** There is no maintained community server to fall back on, so exposing an Amundsen graph to agents means building and owning the integration yourself.
- **Multi-service deployment** of Neo4j or Atlas plus Elasticsearch, now with no upstream behind it.

**If you are running Amundsen today,** Marmot's [Amundsen plugin](https://plugins.marmotdata.io/marmotdata/amundsen) reads an existing metadata graph over Bolt and imports tables, columns, owners, tags, badges, dashboards and lineage. Each table is projected onto the provider and identifier its own technology's plugin already uses, so the import and a later native run land on one asset rather than two. That means you can move off the archived stack without losing the metadata you have built up, and pick the native plugins back up afterwards.
