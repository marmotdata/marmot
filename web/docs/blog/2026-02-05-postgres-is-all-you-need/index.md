...
Data lineage is a graph problem. A dashboard reads from a view, which queries a table, which is populated by an Airflow DAG. You need to traverse these assets relationships in both directions.
...
The database was seeded with 500,000 assets - tables, topics, dashboards, pipelines - with realistic metadata, tags and lineage relationships. Far more than most organizations will catalog.

### Test scenario

k6 simulated 100 concurrent users for 15 minutes, cycling through realistic patterns:

- Plain text search (trigram fuzzy matching)
- Structured search (filtered by type, provider, tags)
- Browse (paginated results with facet counts)
- Asset retrieval and summaries
- Lineage traversal (upstream/downstream dependencies)
...
Most operations respond in under 20ms. Plain text search is the slowest at ~150ms average - that's the cost of trigram fuzzy matching - but it still feels fast to users.
