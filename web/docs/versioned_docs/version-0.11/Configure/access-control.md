...
That instance-wide scope is the thing to understand before you plan anything. In open source Marmot there is no way to say "this team reads only these three tables": whoever holds `assets:view` reads every asset in the catalog. Per-resource grants, inheritance and a policy API are Marmot Cloud and Marmot Enterprise.
