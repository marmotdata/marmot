...
[Pipelines](Pipelines.md) covers schedules, runs and credentials. On Marmot Cloud a pipeline authenticates to Google Cloud, AWS or Azure as itself, or reads the credential from your secret manager at run time.
...
The `*_iam_member`, `*_iam_binding` and `*_iam_policy` resources exist on Marmot Cloud and Marmot Enterprise.
