...
Record agent invocations and read aggregate stats per agent asset. Available in Python and TypeScript today; the Go equivalents are on the roadmap.
...
client.agent_runs.create(
    agent_mrn="marmot://langchain/agent/explorer",
    run_id="run-123",
    ...
    tokens_out=800,
    tool_calls=[
        {"tool_name": "search", "started_at": datetime.now(timezone.utc),
         "status": "success", "target_mrn": "postgres://prod/sales/orders"},
    ],
)

runs = client.agent_runs.list(agent_asset_id, period="24h", limit=25)
stats = client.agent_runs.stats(agent_asset_id, period="7d")
activity = client.agent_runs.activity(agent_asset_id, period="24h")
```
...
await client.agentRuns.create({
  agentMrn: "marmot://langchain/agent/explorer",
  runId: "run-123",
  startedAt: new Date(),
  status: "completed",
  model: "gpt-4o",
  ...
  toolCalls: [
    {
      toolName: "search",
      startedAt: new Date(),
      targetMrn: "postgres://prod/sales/orders",
    },
  ],
});

const runs = await client.agentRuns.list(agentAssetId, {
  period: "24h",
  limit: 25,
});
const stats = await client.agentRuns.stats(agentAssetId, { period: "7d" });
const activity = await client.agentRuns.activity(agentAssetId, {
  period: "24h",
});
```
