...
Everything we make starts as [open source](https://github.com/marmotdata/marmot) and stays that way. [Marmot Cloud](Cloud/index.md) exists for the teams who would rather someone else ran the servers, and it pays for the work on the project everyone else runs for free. We talk to the people who use Marmot directly, on [Discord](https://discord.gg/TWCk7hVFN4) and on GitHub, we write down what we learn on the [blog](/blog), and when you report a bug, an engineer reads it.
...
<DocCardGrid>
  <DocCard
    title="Marmot Cloud"
    description="Let us run it. A managed instance on its own hostname, with fine-grained access control, federated secret stores and keyless pipelines. Free plan, no card."
    docId="Cloud/index"
