...
func TestBuildFilterCondition(t *testing.T) {
	...
	tests := []struct {
		...
		// New tests for @provider field (array)
		{
			...
			expectedCond:     "EXISTS (SELECT 1 FROM unnest(providers) AS elem WHERE lower(elem) = lower($1))",
		...
		},
		{
			...
			expectedCond:     "NOT EXISTS (SELECT 1 FROM unnest(providers) AS elem WHERE lower(elem) = lower($1))",
